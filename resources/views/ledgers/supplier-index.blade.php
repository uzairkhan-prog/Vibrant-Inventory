...
<div class="table-responsive">
    <div class="table-wrapper">
        ...
        <div class="table-title">
            <div class="row">
                <div class="col-md-6">
                    <h2>Supplier Ledger <b>Management</b></h2>
                </div>
                <div class="col-md-6 text-end">
                    <a href="{{ route('supplier-ledgers.create') }}" class="btn btn-secondary">
        ...
        <table class="table table-striped table-hover" id="ledgerTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ \App\Models\Supplier::find($ledger->reference_id)?->name }}</td>
                    <td>{{ number_format($ledger->balance, 2) }}</td>
                    <td>{{ $ledger->updated_at->format('Y-m-d H:i') }}</td>
                    <td>
                        <a href="{{ route('supplier-ledgers.edit', $ledger) }}" class="btn btn-sm btn-success text-white">Edit</a>
                        <form action="{{ route('supplier-ledgers.destroy', $ledger) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete ledger entry?')">
                            @csrf @method('DELETE')
        ...
        @else
        <div class="alert alert-info text-center">No supplier ledger entries found.</div>
        @endif
